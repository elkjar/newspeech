#!/usr/bin/env bash
#
# Build & install vst/vibe as the "VIBE" AU + VST3 plugin via the
# Faust → JUCE pipeline.
#
# Flow:
...
#   2. Patch FaustPluginProcessor.cpp to use our VibeEditor instead
#      of the default FaustPlugInAudioProcessorEditor (one #include +
#      one return-statement swap).
#   3. Projucer --resave regenerates Builds/MacOSX/Vibe.xcodeproj +
...
#   4. xcodebuild the "Vibe - AU" target (universal arm64 + x86_64).
#   5. Replace ~/Library/Audio/Plug-Ins/Components/Vibe.component.
...
PLUGIN_CODE="Vibe"              # AU FourCharCode subtype.
PLUGIN_BUNDLE_ID="com.newspeech.audiounit.Vibe"

INSTALL_NAME="Vibe.component"
INSTALL_DIR="$HOME/Library/Audio/Plug-Ins/Components"

VST3_INSTALL_NAME="Vibe.vst3"
...
# --- 2. Patch FaustPluginProcessor.cpp to use our custom VibeEditor.
#       Two edits: inject the #include before createEditor(), and swap
#       the return-statement to construct VibeEditor instead of the
...
#include "Source/VibeEditor.h"
' \
  -e 's|return new FaustPlugInAudioProcessorEditor (\*this);|return new VibeEditor (*this);|' \
  "$JUCE_PROJECT_DIR/FaustPluginProcessor.cpp"
rm -f "$JUCE_PROJECT_DIR/FaustPluginProcessor.cpp.bak"
grep -q "new VibeEditor" "$JUCE_PROJECT_DIR/FaustPluginProcessor.cpp" || { echo "ERROR: editor patch did not apply"; exit 1; }

# --- 2b. Patch FaustPluginProcessor.cpp to drive Repeat/bpm from the
...
sed -i.bak \
  -e '/    \/\/ MIDI timestamp is expressed in frames/i\
    // Host-tempo override: drive Repeat/bpm from session BPM each block.\
    if (auto* playHead = getPlayHead()) {\
        if (auto pos = playHead->getPosition()) {\
            if (auto hostBpm = pos->getBpm()) {\
                fStateUI.setParamValue("Repeat/bpm", FAUSTFLOAT(*hostBpm));\
            }\
        }\
    }
' \
...
XCODEPROJ="$JUCE_PROJECT_DIR/Builds/MacOSX/Vibe.xcodeproj"
...
xcodebuild \
  ...
  ONLY_ACTIVE_ARCH=NO \
  2>&1 | tail -3
BUILT="$JUCE_PROJECT_DIR/Builds/MacOSX/build/Release/Vibe.component"
...
xcodebuild \
  ...
  ONLY_ACTIVE_ARCH=NO \
  2>&1 | tail -3
BUILT_VST3="$JUCE_PROJECT_DIR/Builds/MacOSX/build/Release/Vibe.vst3"
...
#                in vst/vibe/dist/.signing-config (see .example for
