...
- **tape** — a loop buffer that grabs incoming audio and mangles it: position/length scrubbing, granular layer, hold.
- **repeat** — tempo-synced beat repeat, diced per trigger by `chance`, locked to the host session BPM.
- **reverb** — a Griesinger-style plate to smear whatever survives.

Nothing about it is subtle or transparent. That's the point.

Part of the newspeech plugin suite alongside `saturator`, `glitch`, and `slice`. The UI is the shared kit in [`../common`](../common) — the Sequence app's look.
...
`build.sh` regenerates the JUCE wrapper from `dsp/combined.dsp` via faust2juce, patches in the custom editor (`juce/combined/Source/VibeEditor.*` on top of `../common/ui`), host-tempo override, hidden-parameter filtering, and a zone-backed `getValue()`, then builds universal (arm64 + x86_64) AU and VST3 targets.
