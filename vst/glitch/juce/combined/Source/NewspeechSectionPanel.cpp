...
void NewspeechSectionPanel::paint (juce::Graphics& g)
{
    ...
    // No section border — just the vertical title on the left edge.
    ...
    g.setFont (f);
    // After rotation, text "width" runs along the panel's vertical axis
    // and text "height" runs along the gutter.
...
void NewspeechSectionPanel::resized()
{
    ...
    int x = r.getX();                                     // left-align row
    const int y = r.getY() + (r.getHeight() - h) / 2;     // center row vertically
