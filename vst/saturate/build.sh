#!/usr/bin/env bash
#
# Build & install vst/saturate as the "SATURATE" AU + VST3 plugin.
# Adapted from vst/vibe/build.sh — same Faust → JUCE pipeline, but with
# the default Faust-generated editor (no custom Source/ yet) and the
# host-tempo patch rebound to "LOOP/bpm".
#
# Flow:
#   1. faust2juce regenerates FaustPluginProcessor.cpp from saturate.dsp.
#      The .jucer it ALSO emits is discarded — we keep our own checked-in
#      saturate.jucer.
#   2. Patch FaustPluginProcessor.cpp to use our custom SaturateEditor
...
#   6. Projucer --resave regenerates Builds/MacOSX/Saturate.xcodeproj.
#   7. xcodebuild AU + VST3 (universal arm64 + x86_64).
#   8. Replace ~/Library/Audio/Plug-Ins/Components/Saturate.component
#      and ~/Library/Audio/Plug-Ins/VST3/Saturate.vst3.
...
DSP_FILE="$DSP_DIR/saturate.dsp"
JUCE_PROJECT_DIR="$SCRIPT_DIR/juce/saturate"
...
PLUGIN_NAME="SATURATE"
PLUGIN_MFR="newspeech"
PLUGIN_MFR_CODE="Nwsp"
PLUGIN_CODE="Satu"
PLUGIN_BUNDLE_ID="com.newspeech.audiounit.Saturate"

INSTALL_NAME="Saturate.component"
INSTALL_DIR="$HOME/Library/Audio/Plug-Ins/Components"

VST3_INSTALL_NAME="Saturate.vst3"
...
#       faust2juce also emits — we keep our own checked-in saturate.jucer.
echo "▸ Running faust2juce..."
DSP_TMP_OUTDIR="$DSP_DIR/saturate"
...
[ -f "$JUCE_PROJECT_DIR/saturate.jucer" ] || { echo "ERROR: $JUCE_PROJECT_DIR/saturate.jucer missing — should be checked in"; exit 1; }
...
#        return-statement to construct SaturateEditor instead of the default
...
#include "Source/SaturateEditor.h"
' \
  -e 's|return new FaustPlugInAudioProcessorEditor (\*this);|return new SaturateEditor (*this);|' \
  "$JUCE_PROJECT_DIR/FaustPluginProcessor.cpp"
rm -f "$JUCE_PROJECT_DIR/FaustPluginProcessor.cpp.bak"
grep -q "new SaturateEditor" "$JUCE_PROJECT_DIR/FaustPluginProcessor.cpp" || { echo "ERROR: editor patch did not apply"; exit 1; }
...
#        vst/vibe/build.sh for the long rationale (Logic shows stale
...
xcodebuild \
  ...
  ONLY_ACTIVE_ARCH=NO \
  2>&1 | tail -3
BUILT="$JUCE_PROJECT_DIR/Builds/MacOSX/build/Release/Saturate.component"
[ -d "$BUILT" ] || { echo "ERROR: AU build produced no .component"; exit 1; }

echo "▸ Building universal VST3..."
xcodebuild \
  -project "$XCODEPROJ" \
  -target "Saturate - VST3" \
  -configuration Release \
  ARCHS='arm64 x86_64' \
  ONLY_ACTIVE_ARCH=NO \
  2>&1 | tail -3
BUILT_VST3="$JUCE_PROJECT_DIR/Builds/MacOSX/build/Release/Saturate.vst3"
...
#                 in vst/saturate/dist/.signing-config (see .example for
