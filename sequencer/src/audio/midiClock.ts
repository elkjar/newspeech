...
import { sendMIDIStart, sendMIDIStop } from './midiOut';
...
//
// Pulse emission is native-only: a dedicated Rust thread (see
// src-tauri/src/midi.rs) generates the 0xF8 stream off a monotonic timer; JS
// only sets tempo + start/stop. This is the real path for music work — the
// WebView's setTimeout is too jittery for a 24-PPQN stream and Pam's reads
// that jitter as an unstable tempo. The web build sends Start/Stop only, no
// pulse stream (the per-step web emit was removed with the web strip).
...
// relay: driven by startFollowPlayback at the tracked tempo. No-op on web and
// when no clock-out port is set.
...
// stream KEEPS running so clocked gear holds its time base. Web sends Stop
// directly.
...
// Push tempo changes to the running native clock thread. No-op on web and
