...
- `installMarkers({ params, getSource, getSourceDims, getEdgeData? })` / `tickMarkers(dt)` — source-driven HUD overlay for `N-*.html` pages that take a video/image source. `installMarkers` auto-injects a `data points` slider into `#panel` (default 14, 0–128), runs a sobel pass on `getSource()` at 480px long edge, and pools markers anchored to high-magnitude pixels. Each frame, markers snap to the local edge center-of-mass (so they track features as the source moves) and are drawn as `[xyz]` labels with nearest-neighbor connectors above whatever the page just rendered. `getEdgeData` is an optional opt-out: pages that already compute their own sobel (`10-edges.html`, `12-edgemask.html`) pass `() => ({mag, w, h})` to share the buffer and avoid a duplicate readback. Call `installMarkers` BEFORE `installPanel` so its slider gets wired alongside the page's. Call `tickMarkers(dt)` at the end of the page's render path so markers sit on top.

**Parameter panel.** Toggle with `0`; sits at `bottom:32px right:32px`. CSS injected by `core.js`. Each page declares a `params` object and a `<div id="panel">` containing one `<label>` per slider with `<span class="name">`, `<span class="val">`, and `<input type="range" data-k="...">`. Page reads `params.foo` directly. For params that need a side-effect on change (e.g. retarget a particle pool), set `window.onParamChange = (k) => { ... }` before calling `installPanel`. `installPanel` also appends a `.panel-globals` block at the bottom with global render controls — a `grayscale: on/off` tap-row, a `contrast` slider (range 0.5–2.5, default 1.0), a `grid: off/dots/grid/topo` tap-row that cycles a static background grid style, and a `grid opacity` slider (0–1, default 0.5). Grayscale + contrast compose into a single CSS filter on `canvas#bg` via the `--ns-canvas-filter` custom property, so they work on mobile Safari (where `ctx.filter` silently no-ops pre-18). State persists in `localStorage` under `newspeech.grayscale` / `newspeech.contrast` / `newspeech.gridStyle` / `newspeech.gridOpacity`. The contrast and grid-opacity sliders are wrapped in `<div class="panel-row">` (not `<label>`) and carry `data-global="1"` so they're excluded from MIDI slot indexing — knobs continue to map to per-page sliders only.
