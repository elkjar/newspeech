...
//   email       required
//   source      "homepage" | "plugins" | "night-school"
...
import { client, normalizeEmail, PLUGINS, SOURCES, TRACK_RE } from "../lib/resend.mjs";
...
export default async (req) => {
  ...
  try {
    const r = await resend.subscribe({ email, source, plugin, version, track });
    console.log(`subscribe: ${r.created ? "new" : "known"} ${source}${plugin ? " " + plugin : ""}${track ? " " + track : ""}`);
